...
export const createWorkersShiftSchedule = async (req, res) => {
    ...
    try {
        ...
        const staffUsernameArray = staff.map(member => member.username);
        ...
        const newWorkersShiftSchedule = new WorkersShiftSchedule({ 
            ...
            staff: staff.map(member => ({ username: member.username,type: member.type, id: member.id.toString() })) // Ensure staff.name and staff._id are valid strings
        });
        ...
        newWorkersShiftSchedule.username = staffUsernameArray.join(", ");
