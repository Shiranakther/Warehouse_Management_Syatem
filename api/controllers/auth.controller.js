...
export const deletePendingSupplier = async (req, res, next) => {
  const { supplierId } = req.params; // Assuming the ID of the pending supplier to be deleted is provided in the request params

  try {
    // Find the pending supplier by ID and delete it
    await PendingSupplier.findByIdAndDelete(supplierId);
    
    res.status(200).json('Pending supplier deleted successfully');
  } catch (error) {
    next(error);
  }
};
...
export const supplierApprove = async (req, res, next) => {
  const { supplierName, email, phoneNumber, companyName, password } = req.body;

  try {
    // Hash the password
    const hashedPassword = await bcryptjs.hash(password, 10); // 10 is the saltRounds

    // Create a new supplier with hashed password
    const newSupplier = new Supplier({ supplierName, email, phoneNumber, companyName, password: hashedPassword });
    
    // Save the new supplier to the database
    await newSupplier.save();

    res.status(201).json('Supplier registration Success');
  } catch (error) {
    next(error);
  }
};
