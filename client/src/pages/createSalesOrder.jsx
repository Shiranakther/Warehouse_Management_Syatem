...
//         await axios.post('api/order/createOrder', newOrder);
//         alert('Order Created');
  ...
  return (
    <div className="absolute flex h-screen justify-center w-9/12 ml-80 mt-14 bg-gray-300 z-0 pt-10">
      <div className="w-full max-w-3xl">
            ...
            {orderItems.map((order, index) => (
              <tr key={index}>
                ...
                <td className="px-4 py-2 border border-gray-300">
                  {AllItems.find(item => item.ItemID === order.ItemCode)?.ItemType || 'Item Name'}
                </td>
                <td className="px-4 py-2 border border-gray-300">{order.Quantity}</td>
              </tr>
            ))}
        ...
        <div className="flex justify-between">
          <button type="button" className="bg-blue-500 text-white px-4 py-2 rounded-md focus:outline-none focus:bg-blue-600 mr-4" onClick={handleCreateOrder}>Create Order</button>
        </div>
      </div>
    </div>
  );
