...
export default function Addworkers() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      dispatch(registerSuccess(data.staff)); // Ensure backend returns 'staff'
      setFormData({ type: '' }); // Reset form, keeping type initialized
    ...
    <div className="flexy">
      <div className="p-3 max-w-lg mx-auto w-4/5 mr-96">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <input
            type="number"
            placeholder="License (Driver Only)"
            id="license"
            className={`bg-slate-100 p-3 rounded-lg border-2 border-zinc-400 ${
              formData.type === 'Supervisor' || formData.type === 'Labor' ? 'opacity-50 cursor-not-allowed' : ''
            }`}
          ...
          <input
            ...
            id="password"
            className={`bg-slate-100 p-3 rounded-lg border-2 border-zinc-400 ${
              formData.type === 'Driver' || formData.type === 'Labor' ? 'opacity-50 cursor-not-allowed' : ''
            }`}
        ...
        <Link to="/Workerlist">
          <div className="bg-slate-600 text-white p-1 rounded-lg uppercase hover:opacity-95 text-center mt-5 w-32 ml-0.5">
            Worker List
          </div>
        </Link>
        {error && <p className="text-red-700 mt-5">{error.message || error || 'Something went wrong!'}</p>}
      </div>
    </div>
  );
}